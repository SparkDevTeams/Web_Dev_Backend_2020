...
export default {
  ...
  list_all_challenges: (req, res) => {
    ...
    // Query database
    Challenge.find({}, projection(req.user, "GET /challenges"), (err, dbData) => {
  ...
  read_a_challenge: (req, res) => {
    ...
    // Query database
    Challenge.findById(req.params.id, projection(req.user, "GET /challenges/:id"), (err, dbData) => {
      // If error occured, return error response
      if (err) {
        res.status(502).send({});
      } else if (dbData == null) {
  ...
  update_a_challenge: (req, res) => {
    ...
    //these are fields you do not want users to update
    ...
    // Cast documentToUpdate as a Challenge (to facilitate unit testing)
